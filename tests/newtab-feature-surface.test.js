...
assert.match(app, /function mutateSidebar\(mutator, done\)[\s\S]*?sbMutationQueue/);
assert.match(app, /function sbReadStored\(done\)[\s\S]*?chrome\.storage\.local\.get\(KEY/);
assert.match(app, /sbMutationQueue/);
assert.match(app, /SB_MUTATION_MAX_ATTEMPTS/);
assert.match(app, /sbStorageError[\s\S]*?runtime\.lastError/);
assert.match(app, /sbStamp[\s\S]*?afterWrite\.stamp/);
assert.match(app, /删除失败，已恢复最新数据/);
assert.match(app, /function sbFocusCard\(id\)[\s\S]*?\.focus\(\)/);
...
assert.match(content, /function mutateSidebar\(mutator, done\)[\s\S]*?sidebarMutationQueue/);
assert.match(content, /function readStoredState\(done\)[\s\S]*?chrome\.storage\.local\.get\(KEY/);
assert.match(content, /sidebarMutationQueue/);
assert.match(content, /SIDEBAR_MUTATION_MAX_ATTEMPTS/);
assert.match(content, /storageError[\s\S]*?runtime\.lastError/);
assert.match(content, /storageStamp[\s\S]*?afterWrite\.stamp/);
assert.match(content, /删除失败，已恢复最新数据/);
assert.match(content, /function focusCard\(id\)[\s\S]*?\.focus\(\)/);
